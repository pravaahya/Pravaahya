...
export default function AdminOrders() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const res = await fetch("https://backend-rho-brown-23.vercel.app/api/orders", {
  ...
  const handleStatusUpdate = async (id: string, newStatus: string) => {
    try {
      ...
      const res = await fetch(`https://backend-rho-brown-23.vercel.app/api/orders/${id}`, {
        method: "PUT",
        headers: {
                      ...
                      <td className="px-6 py-5">
                         ...
                         {!isUnpaid ? (
                            <div className="flex flex-col gap-2 mt-3 w-max">
                               <a href={`https://backend-rho-brown-23.vercel.app/api/orders/${order._id}/invoice`} target="_blank" rel="noopener noreferrer" className="inline-block text-[10px] text-center font-bold text-blue-600 hover:text-blue-800 uppercase tracking-widest border border-blue-200 bg-blue-50 hover:bg-blue-100 px-3 py-1 rounded-full transition-colors">
                                  View Invoice
                               </a>
                               <a href={`https://backend-rho-brown-23.vercel.app/api/orders/${order._id}/invoice?download=true`} className="inline-block text-[10px] text-center font-bold text-green-600 hover:text-green-800 uppercase tracking-widest border border-green-200 bg-green-50 hover:bg-green-100 px-3 py-1 rounded-full transition-colors">
                                  Download Invoice
                               </a>
                            </div>
                         ) : (

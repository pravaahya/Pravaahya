...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  let related: Product[] = [];
  try {
     const backendRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:5000/api"}/products/${id}`, { cache: 'no-store' });
     ...
     if (backendJson.success && backendJson.data) {
        ...
        const relatedRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:5000/api"}/products`, { cache: 'no-store' });

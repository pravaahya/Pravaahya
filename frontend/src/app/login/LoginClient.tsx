...
export function LoginClient() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://backend-rho-brown-23.vercel.app/api/auth/send-otp`, {
        method: "POST",
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://backend-rho-brown-23.vercel.app/api/auth/verify-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },

...
exports.validateGuestCheckout = void 0;
const validateGuestCheckout = (req, res, next) => {
    const { user } = req.body;
    ...
    if (typeof houseNo !== 'string' || houseNo.trim().length < 1) {
        res.status(400).json({ error: "houseNo -> required, min 1 char" });
        return;
    }
    if (typeof streetArea !== 'string' || streetArea.trim().length < 3) {
        res.status(400).json({ error: "streetArea -> required, min 3 chars" });
        return;
    }
    if (landmark && typeof landmark === 'string' && landmark.trim().length > 100) {
        res.status(400).json({ error: "landmark -> max 100 chars" });
        return;
    }
    if (typeof cityVillage !== 'string' || !/^[a-zA-Z\s]+$/.test(cityVillage.trim())) {
        res.status(400).json({ error: "cityVillage -> required, letters only" });
        return;
    }
    ...
    if (typeof pincode !== 'string' || !/^\d{5,10}$/.test(pincode.trim())) {
        res.status(400).json({ error: "pincode -> required, 5-10 digits (numeric only)" });
        return;
    }
